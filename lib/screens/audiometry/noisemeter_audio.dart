...
import 'dart:math';
import 'setting_audio.dart';

/// 环境噪声评估页--声场校准使用
class NoiseMeterAudioPage extends StatefulWidget {
  /// 模式
  final int Mode;
  NoiseMeterAudioPage({Key? key, this.Mode = 0}) : super(key: key);
  @override
  NoiseMeterAudioPageState createState() => NoiseMeterAudioPageState();
}

class NoiseMeterAudioPageState extends State<NoiseMeterAudioPage> {
  Timer? _timer;
  ...
  void stopCollect() async {
    try {
      ...
      setState(() {
        int sumLength = _meanDecibel.length - 10;
        _isRecording = false;
        curentTimer = 6000;
